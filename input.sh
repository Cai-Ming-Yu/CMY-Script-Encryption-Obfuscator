...
makeNewFile() {
    ...
    until [[ "${num}" -eq 0 ]]; do
        ...
        local shell="{ \$(printf \"$(printf "set +x" | bzip2 -c9 | base64 -w 0)\"|base64 -d|bzcat);\$(printf \"$(printf "eval" | gzip -c9 | base64 -w 0)\"|\$(printf \"$(printf "base64 -d" | base64 -w 0)\"|base64 -d)|\$(printf \"$(printf "zcat" | bzip2 -c9 | base64 -w 0 )\"|base64 -d|bzcat)) \"\$(echo -en \"${shell}\"|base64 -d|zcat)\";};"
        local shell="{ set +x;\$(printf \"$(printf "eval" | bzip2 -c9 | base64 -w 0)\"|\$(printf \"$(printf "base64 -d" | base64 -w 0)\"|base64 -d)|\$(printf \"$(printf "bzcat" | base64 -w 0)\"|base64 -d)) \"\$(echo -en \"$(echo -en "${shell}" | base64 -w 0)\"|base64 -d)\";};"
        let num--
    done
    [[ -f "${output}" ]] && {
